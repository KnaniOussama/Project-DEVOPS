...
npm install
# If you need to run migrations for TypeORM (SQL), configure .env and run:
...
*   Frontend Deployment and Service (NodePort on port 30080)
...
Prometheus and Grafana are deployed using the `kube-prometheus-stack` Helm chart. The `prometheus-grafana-values.yaml` file was used for custom configuration.
...
Grafana is configured with an ingress, so you can access it via a hostname. You'll need to add an entry to your local `hosts` file (e.g., `/etc/hosts` on Linux/macOS or `C:\Windows\System32\drivers\etc\hosts` on Windows).
...
*   **Username**: `admin`
*   **Password**: `prom-operator` (as configured in `prometheus-grafana-values.yaml` - **change this in a real scenario!**)
...
Provide screenshots for the following:

*   **Docker Compose**: Application running locally via `docker compose up`.
*   **Jenkins Pipeline**: Successful CI pipeline run, showing all stages (Build, Scan, Push).
*   **Kubernetes Pods & Services**: `kubectl get pods` and `kubectl get services` output after Helm deployment.
*   **Frontend Application on Kubernetes**: Screenshot of the frontend accessible via NodePort.
*   **Grafana Dashboards**: Screenshots of your custom application dashboard and a cluster resource dashboard.
